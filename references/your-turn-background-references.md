...
- Place a small, crisp felt version of the approved icon on the right. Keep it comfortably inside the canvas rather than edge-cropping the foreground icon.
- Place a much larger, very faint tonal version of the same silhouette behind the foreground icon as an intentionally cropped watermark. Preserve its broad recognizable shape, but keep it subordinate to the foreground icon and UI.
...
| Hangman | [hangman.webp](https://asset-data.kulfiapp.com/media/home_game_v2/daily_game_your_turn_bg/hangman.webp) |
